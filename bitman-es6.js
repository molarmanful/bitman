...
log=_=>stats.innerHTML=`Code          │ ${format=code.replace(/[\x00-\x1f]/g,x=>String.fromCharCode(x.charCodeAt()+9216)).replace(/</g,'&lt;').replace(/>/g,'&gt;').split(/(&[gl]t;)|/).filter(x=>x),format[ip]=`<span style=background-color:#7ec0ee>${format[ip]||""}</span>`,format.join``.replace(/</g,'&lt;').replace(/>/g,'&gt;')}
IP            │ ${ip}
Bits          │ ${bits}
